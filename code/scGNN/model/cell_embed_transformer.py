...
class CellEmbedTransformer(torch.nn.Module):
    ...
    '''
    def __init__(self, hidden, heads = 8, num_layers = 1, max_num_genes = 10000, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.hidden = hidden
        self.heads = heads
        self.num_layers = num_layers
        self.transformer = nn.ModuleList([torch.nn.TransformerEncoderLayer(d_model=hidden, dim_feedforward = 512, nhead=heads, batch_first=True) for _ in range(num_layers)])
